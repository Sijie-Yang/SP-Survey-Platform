...
function classifyClause(clause) {
  ...
  if (PAGE_COUNT.test(clause) && !DRAFT_ACTION.test(clause)) return 'answer';
...
export function annotateHistoryForModel(messages = []) {
  return (messages || []).filter((item) => item?.content).map((item) => {
    if (item.role !== 'user') {
      return {
        role: item.role,
    ...
    const status = item.metadata?.taskStatus
      || (item.metadata?.questionModeWriteRefused ? 'rejected' : 'unknown');
    return {
      role: 'user',
      content: `[Prior task — ${status}. Background only. Do not execute this request unless the current message explicitly continues it.]\n${item.content}`,
    };
  });
}

export function currentTaskMessage(message, intent = classifyUserIntent(message)) {
  const goals = (intent.goals || [intent.goal || 'answer']).join(', ');
  return `[Current task — this is the only request to execute now. Goals: ${goals}.]\n${textOf(message)}`;
}

export function initialLoadingStatus(assistantMode, intent = {}) {
